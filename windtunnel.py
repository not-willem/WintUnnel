...
button3 = Button(screen, 180, 10, 95, 40, text="Quit", onClick=lambda: pygame.quit(), inactiveColour="white", shadowDistance=4, shadowColour="black")
...
def add_wind_source():
    ...
    if wind_source_rect is None:
        wind_source_rect = pygame.Rect(322, 322, 60, 60)
...
while running:
    ...
    for i2 in range(14):
        for i in range(14):
            pygame.draw.rect(screen, "black", (i * 50, i2 * 50, 50, 50), 2)
    if wind_source_active and wind_source_rect is not None:
        pygame.draw.rect(screen, "black", wind_source_rect)
        pygame.draw.circle(screen, "red", (wind_source_rect.x, wind_source_rect.y), 4)
